...
function simulate( energy = 5000.0, N = 10000, para = rand(10); MMM::Int = 1 , dens::Float64 = 3.5e7, temp::Float64 = 75.0, Q::Float64 = 1.0)
    ...
    diff_size = N*200
    diff_array = Array{Float64}(diff_size)
    counti = 1
    flag = false
    ...
    for i in 1:N   #""" THE loop """
        ...
        while currene>0.0   #""" Simulates life of a particle """
            ...
            elseif a < (thresholdps + thresholddi + thresholdex)
             ...
             # diff_array[counti] = diff_array[counti] - currene
            if counti >= diff_size
                flag = true
                break;
            end
...
@time dataqwe = simulate(varenergy , particles, para, MMM = 1 , dens = vardensity, temp = vartemp, Q=q)
...
savefig(plott,"Energy loss "*string(vartemp))
plott = plot(dataqwe[(dataqwe.<1000.0).&(dataqwe.!=0.0)],title = "Temperature :  " * string(vartemp) *" K", xlabel = "collisions", ylabel = "Energy lost on collisions(eV)")

histogram(dataqwe[dataqwe.!=0],nbins=100)
#
